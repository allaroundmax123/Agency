...
; # 														                                                                    #
; # SAMP-Funktionen:                                                                                                            #
; #     - IsSAMPAvailable()                         PrÃƒÂ¼ft, ob man in den Chat schreiben kann & ob GTA geladen ist            #
; #     - isInChat()                                PrÃƒÂ¼ft, ob der Spieler gerade chattet oder in einem Dialog ist            #
...
; #     - addChatMessage(wText)                     FÃƒÂ¼gt eine Zeile in den Chat ein (nur fÃƒÂ¼r den Spieler sichtbar)        #
...
; #	- getDialogCaption() - Liest die Ãœberschrift des (zuletzt) angezeigten Dialogs aus 			                        	#
...
; #     - getStreamedInPlayersInfo()                Zeigt Informationen ÃƒÂ¼ber die gestreamten Spieler.                        #
; #     - callFuncForAllStreamedInPlayers()         FÃƒÂ¼hrt bestimmte Funktionen, fÃƒÂ¼r die gestreamten Spieler aus.          #
...
; #     - getPlayerArmour()                         Ermittelt den RÃƒÂ¼stungswert des Spielers                                  #
...
; #	    - getPlayerState()			                Ermittelt den "Status" des Spielers (Zu FuÃƒÅ¸, Fahrer, Tot)                #
...
; #		- IsPlayerinAnyInterior()					Ermittelt ob der Spieler in einem Inerior ist
; # 														                                                                    #
; ###############################################################################################################################
; # 														                                                                    #
; # Fahrzeugfunktionen:                                                                                                         #
...
; #		- GetSeatIDs()								Ermittelt wer in deinem Fahrzeug sitzt										#
; # 														                                                                    #
; ###############################################################################################################################
; # 														                                                                    #
; # Standpunktbestimmung:                                                                                                       #
; #     - getCoordinates()                          Ermittelt die aktuelle Position (Koordinaten)                               #
; #	    - getPlayerPos(X,Y,Z) 			            siehe oben drÃƒÂ¼ber 						                                #
; # 														                                                                    #
; # --------------------------------------------------------------------------------------------------------------------------- #
; # 														                                                                    #
; #     - initZonesAndCities()                      Initialisiert eine Liste aller Standartgebiete                              #
; #                                                 (Voraussetzung fÃƒÂ¼r die folgenden Funktionen dieser Kategorie)            #
...
; #     - AddZone(Name, X1, Y1, Z1, X2, Y2, Z2)     FÃƒÂ¼gt eine Zone zum Index hinzu                                           #
; #     - AddCity(Name, X1, Y1, Z1, X2, Y2, Z2)     FÃƒÂ¼gt eine Stadt zum Index hinzu                                          #
; #	    - IsPlayerInRangeOfPoint(X, Y, Z, Radius)   Bestimmt ob der Spieler in der NÃƒÂ¤he der Koordinaten ist                  #
; #	    - IsIsPlayerInRangeOfPoint2D(X, Y, Radius)  Bestimmt ob der Spieler in der NÃƒÂ¤he der Koordinaten ist                  #
; #	    - getPlayerZone()                   				                                                                    #
; #	    - getPlayerCity()                   					                                                                #
; # 														                                                                    #
; ###############################################################################################################################
; # Sonstiges:                                                                                                                  #
; #     - checkHandles()                                                                                                        #
; #     - AntiCrash()								Hilft gegen das abstÃƒÂ¼rzen bei Warningscodes                              #
; ###############################################################################################################################
...
GetSeatIDs()
{
    ...
    if (dw) {
        ...
        return [getIdByPed(driver), getIdByPed(psg1), getIdByPed(psg2), getIdByPed(psg3)]
    } else {
        AddChatMessage(PREFIX . "Du befindest dich nicht in einem Fahrzeug!")
    }
}
...
getTargetVehicleSpeedByPed(dwPED) {
    ...
    fVehicleSpeed := (fVehicleSpeed * 100) * 1.43           ;Der Wert "1.43" ist meistens auf jedem Server anders. Die Geschwindigkeit wird somit erhÃƒÂ¶ht bzw. verringert
...
GetChatLine(Line, ByRef Output) {
    ...
    Output := readString(hGTA, dwAdress + offset, 128)
    return
}
...
sTitle := readString(hGTA, dwAddress + SAMP_DIALOG_TITLE, SAMP_DIALOG_TITLE_SIZE)
return sTitle
}
...
updateStatsDialog()
{
stats := getDialogContent()
if(InStr(stats, "Personalien")){
statsName := GetDialogTitle()
statsID := getPlayerIdByName(statsName)
RegExMatch(stats, "(.*)Morde\: ([0-9]+)(.*)", morde)
RegExMatch(stats, "(.*)Gestorben\: ([0-9]+)(.*)", tode)
RegExMatch(stats, "(.*)Respekt\: ([0-9]+)\/([0-9]+)(.*)", respekt)
RegExMatch(stats, "(.*)Level\: ([0-9]+)(.*)", level)
RegExMatch(stats, "(.*)Bargeld\: ([0-9]+)\$(.*)", bargeld)
RegExMatch(stats, "(.*)Konto\: ([0-9]+)\$(.*)", konto)
RegExMatch(stats, "(.*)Festgeld\: ([0-9]+)\$(.*)", festgeld)
RegExMatch(stats, "(.*)Partner\: (\S+)(.*)", partner)
RegExMatch(stats, "(.*)Payday\: ([0-9]+)\/([0-9]+)(.*)", payday)
paydayMinutes := 60 - payday2
respektPoints := respekt3 - respekt2
partnerID := GetPlayerIdByName(partner2)
levelProzent := respekt2 * 100
levelProzent := round(levelProzent / respekt3) . "%"
geldGesamt := FormatNumber(bargeld2 + konto2 + festgeld2)
bargeld := FormatNumber(bargeld2)
konto := FormatNumber(konto2)
festgeld := FormatNumber(festgeld2)
if(tode2 == 0){
KD = %morde2%.00
}else{
KD := morde2 / tode2
...
StringReplace, stats, stats, Geschlecht:, {FFFF00}Geschlecht:{FFFFFF}
if(partnerID == -1){
StringReplace, stats, stats, Partner:, {FFFF00}Partner:{FFFFFF}
}else{
StringReplace, stats, stats, Partner: %partner2%, {FFFF00}Partner:{FFFFFF} %partner2% [ID: %partnerID%]
}
StringReplace, stats, stats, Level: %level2%, {FFFF00}Level:{FFFFFF} %level2% (%levelProzent%)
StringReplace, stats, stats, Respekt: %respekt2%/%respekt3%, {FFFF00}Respekt:{FFFFFF} %respekt2%/%respekt3% (noch %respektPoints% Punkte)
StringReplace, stats, stats, Payday: %payday2%/60, {FFFF00}Payday:{FFFFFF} in %paydayMinutes%
StringReplace, stats, stats, Fraktion:, {FFFF00}Fraktion:{FFFFFF}
StringReplace, stats, stats, Rang:, {FFFF00}Rang:{FFFFFF}, All
StringReplace, stats, stats, Nebenjob:, {FFFF00}Nebenjob:{FFFFFF}
StringReplace, stats, stats, Organisation:, {FFFF00}Organisation:{FFFFFF}
StringReplace, stats, stats, Bargeld: %bargeld2%$, {FFFF00}Bargeld:{FFFFFF} %bargeld%$
StringReplace, stats, stats, Konto: %konto2%$, {FFFF00}Konto:{FFFFFF} %konto%$
StringReplace, stats, stats, Festgeld: %festgeld2%$, {FFFF00}Festgeld:{FFFFFF} %festgeld%$
StringReplace, stats, stats, Zinssatz: 3.50, {FFFF00}Zinssatz:{FFFFFF} 3.50`n`t`t{FFFF00}Gesamt:{FFFFFF} %geldGesamt%$
StringReplace, stats, stats, Morde:, {FFFF00}Morde:{FFFFFF}
StringReplace, stats, stats, Gestorben:, {FFFF00}Gestorben:{FFFFFF}
StringReplace, stats, stats, Verbrechen:, {FFFF00}K/D: {FFFFFF}%KD%`n`t`t{FFFF00}Verbrechen:{FFFFFF}
StringReplace, stats, stats, Spielzeit:, {FFFF00}Spielzeit:{FFFFFF}
StringReplace, stats, stats, Knast:, {FFFF00}Knast:{FFFFFF}
StringReplace, stats, stats, Wanteds:, {FFFF00}Wanteds:{FFFFFF}
StringReplace, stats, stats, Verwarnungen:, {FFFF00}Verwarnungen:{FFFFFF}
StringReplace, stats, stats, Materialien:, {FFFF00}Materialien:{FFFFFF}
StringReplace, stats, stats, Safe:, {FFFF00}Safe:{FFFFFF}, All
StringReplace, stats, stats, Drogen:, {FFFF00}Drogen:{FFFFFF}
StringReplace, stats, stats, Drogendealer:, {FFFF00}Drogendealer:{FFFFFF}
StringReplace, stats, stats, Angler:, {FFFF00}Angler:{FFFFFF}
StringReplace, stats, stats, Busfahrer:, {FFFF00}Busfahrer:{FFFFFF}
StringReplace, stats, stats, Anwalt:, {FFFF00}Anwalt:{FFFFFF}
StringReplace, stats, stats, Detektiv:, {FFFF00}Detektiv:{FFFFFF}
StringReplace, stats, stats, Pilot:, {FFFF00}Pilot:{FFFFFF}
StringReplace, stats, stats, Hochseefischer:, {FFFF00}Hochseefischer:{FFFFFF}
StringReplace, stats, stats, Exp:, {FFFF00}Exp:{FFFFFF}, All
ShowDialog(0, statsName . " [ID: " . statsID . "]", "{FFFFFF}" . stats, "SchlieÃŸen")
}
}
...
getSkinFraction(id) {
	skins := {"LSPD": [163, 164, 265, 266, 267, 280, 281, 283, 284, 288, 194], "FBI": [165, 166, 286, 294, 194], "Sanitäter": [70, 274, 275, 276, 193], "Feuerwehr": [255, 277, 278, 279, 191], "Russen Mafia": [111, 112, 113, 124, 125, 126, 127, 272, 40], "Yakuza Mafia": [122, 123, 186, 203, 204, 228, 169, 224], "Hitman": [], "Wheelman": [], "San News": [60, 170, 188, 227, 240, 250, 56, 226], "Grove Street": [105, 106, 107, 269, 269, 271, 65], "Ballas": [102, 103, 104, 293, 13], "Los Chickos Malos": [46, 47, 48, 98, 185, 223, 214], "Ordnungsamt": [8, 50, 71, 233], "Transport GmbH": [34, 44, 132, 133, 202, 206, 261, 31, 131], "San Fierro Rifa": [114, 115, 116, 173, 174, 175, 184, 273, 195, 198], "Los Santos Vagos": [108, 109, 110, 292, 91], "Triaden": [117, 118, 120, 208, 263]}
	fraction := ""
